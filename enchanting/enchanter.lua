...
local function ToRoman(num)
    ...
    local result = "";
    for _, numeral in next, ROMAN_LITTERALS do
        while (num >= numeral.value) do
    ...
    return setmetatable(list, {
        __tostring = function(self)
            ...
            for _, item in ipairs(self) do
                buf = buf .. SnakeToPascal(item.Name) .. " " .. tostring(ToRoman(item.Level)) .. "\n"
            end
...
local function EnchantProfile(bookshelvesAround)
    ...
    for _, value in ipairs(ORDER) do
        ...
        print(SnakeToPascal(selected.Name) .. " " .. tostring(ToRoman(selected.Level)))
    end
end
